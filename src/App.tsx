...
import { MdOutlineCheck, MdDeleteOutline, MdCreate, MdClose } from 'react-icons/md';
...
function App (): React.ReactElement {
  ...
  const [tabsTodo, setTabsTodo] = useState<Todos[] | []>([]);
  ...
  const handleUpdateTabs = (tab: Action): void => {
    if (tab.type === 'completed') {
      setTodos(tabsTodo.filter((item) => item.completed));
        ...
        <div className="w-3/4 h-min flex flex-col gap-6 absolute top-12 translate-x-[15%]">
          ...
          <div className="flex justify-between rounded-md">
            <div className="w-3/4 pr-2">
              <input value={newTodo} onChange={handleChangeInputTodo} type="text" placeholder='Input your todo' className="w-full bg-white text-xl text-dark mt-1 px-4 py-3 shadow-lg rounded-lg focus:outline-none focus:ring-primary focus:ring-2 focus:border-primary"/>
            </div>
                    ...
                    <div className="flex gap-4">
                      {
                        !item.completed
                          ...
                          : (
                          <div className="flex items-center p-2 border border-red-500 rounded hover:cursor-pointer">
